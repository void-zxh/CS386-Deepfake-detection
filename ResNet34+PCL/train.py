...
def videotest(val_loader, model, args):
    ...
    cou=[]
    lio=518
    for i in range(0,lio):
        if i+1> 178:
            final_y_true.append(0)
        else:
            final_y_true.append(1)
        final_y_score.append(0)
        cou.append(0)
    ...
    with torch.no_grad():
        ...
        for inputs, labels, video_names in val_loader:
            ...
            li = len(video_names)
            for i in range(0,li):
                toid=int(video_names[i]-1)
                final_y_score[toid] = final_y_score[toid] + y_score[i]
                cou[toid]= cou[toid] +1
    ...
    for i in range(0,lio):
        if cou[i]>0:
            final_y_score_l.append(final_y_score[i]/cou[i])
            final_y_true_l.append(final_y_true[i])
        else:
            final_y_score_l.append(final_y_true[i])
            final_y_true_l.append(final_y_true[i])

    final_y_true_l=np.array(final_y_true_l)
    final_y_score_l=np.array(final_y_score_l)
    throughput = 1.0 / (batch_time.avg / inputs.size(0))
    re_auc=roc_auc_score(final_y_true_l, final_y_score_l)
