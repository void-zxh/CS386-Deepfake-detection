...
class DataSetGenerator():
    ...
    def pre_work(self):
        ...
        for i in range(0,data_len):
            dir=str(i)
            landmarks_db_path=landmark_path+dir.zfill(3)+'/landmark_db.txt'
            image_path=img_dir_path+dir.zfill(3)+'/'
            landmarks_sub_db = self._read_landmarks_pairs(landmarks_db_path)
            image_sub_names = self._get_images(image_path)
            for key,value in landmarks_sub_db.items():
                new_key=dir.zfill(3)+'/'+key
                self.landmarks_db[new_key]=value
            for image_name in image_sub_names:
                new_image_name=dir.zfill(3)+'/'+image_name
                self.image_names.append(new_image_name)
        print(len(self.landmarks_db))
    ...
    def get_blended_face(self, background_face_path):
        ...
        # 2. get nearest face
        foreground_face_path = ut.get_nearest_face_cross(background_face_path,
                                                   self.landmarks_db)
    ...
    def create_dataset(self):
        ...
        for img_name in tqdm(self.image_names):
            ...
            im.save(self.data_set_path
                    ...
                    +file_name+"_"+img_name.split(".")[0]
                    + ("_fake" if fake else "_real")+".jpg",
            ...
            im_mask.save(self.data_set_path
                    + ("/fake_mask/" if fake else "/real_mask/")
                    +file_name+"_"+ img_name.split(".")[0]
                    + ("_fake" if fake else "_real")+".jpg",
                    quality=random.randint(80, 100))
